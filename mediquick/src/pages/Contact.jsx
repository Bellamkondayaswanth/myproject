...
export default function Contact() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post('http://localhost:5000/api/email/contact', {
        name: form.name,
        email: form.email,
  ...
  if (sent) return (
    <div style={{
      ...
      <button onClick={() => { setSent(false); setForm({ name: '', email: '', subject: '', message: '' }); }} style={{
        ...
        padding: '12px 32px', borderRadius: '12px', fontWeight: 600, border: 'none', cursor: 'pointer'
      }}>Send Another</button>
    </div>
  );
          ...
          <div style={{
            ...
            <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <input name="subject" placeholder="How can we help?" value={form.subject} onChange={handle} required style={inputStyle}
                  onFocus={e => e.target.style.borderColor = 'var(--primary)'}
                  onBlur={e => e.target.style.borderColor = 'var(--border)'} />
              </div>
              <div>
                <label style={{ fontSize: '13px', fontWeight: 600, display: 'block', marginBottom: 6 }}>Message</label>
                <textarea name="message" placeholder="Write your message..." value={form.message} onChange={handle} required rows={5}
