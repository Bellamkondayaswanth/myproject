            ...
            {orders.map(o => {
              ...
              return (
                <div key={o._id} onClick={() => setSelected(o)} style={{ background: selected?._id === o._id ? 'var(--primary-light)' : 'white', border: `1px solid ${selected?._id === o._id ? 'var(--primary)' : 'var(--border)'}`, borderRadius: '14px', padding: '16px', cursor: 'pointer' }}>
                  ...
                  <div style={{ fontSize: '13px', fontWeight: 600, marginBottom: 2 }}>👤 {o.customer?.name || 'Customer'}</div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: 4 }}>{o.customer?.email}</div>
                  ...
                  return (
                    <div key={step} style={{ display: 'flex', alignItems: 'center', flex: i < steps.length - 1 ? 1 : 0 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: i < stepNum ? 'var(--primary)' : 'var(--border)', color: i < stepNum ? 'white' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: 700 }}>{i < stepNum ? '✓' : i + 1}</div>
