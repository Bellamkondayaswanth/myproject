  ...
  const updateStatus = async (newStatus) => {
    try {
      ...
      const res = await axios.patch(
        `http://localhost:5000/api/orders/${selected._id}/status`,
        { status: newStatus },
        { headers }
      );
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', padding: '40px 2rem' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: '2rem', marginBottom: 8 }}>📦 Manage Orders</h1>
        <p style={{ color: 'var(--text-muted)', marginBottom: 32 }}>{orders.length} total orders</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '80vh', overflowY: 'auto' }}>
            {orders.map(o => {
              ...
              return (
                <div key={o._id} onClick={() => setSelected(o)} style={{
                  background: selected?._id === o._id ? 'var(--primary-light)' : 'white',
                  border: `1px solid ${selected?._id === o._id ? 'var(--primary)' : 'var(--border)'}`,
          ...
          {selected && (
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid var(--border)', padding: '28px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 700, padding: '4px 14px', borderRadius: '20px', ...statusColor(selected.status) }}>
                  {selected.status}
                </span>
              </div>
                    ...
                    <div key={step} style={{ display: 'flex', alignItems: 'center', flex: i < steps.length - 1 ? 1 : 0 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                        <div style={{
                          ...
                          background: i < stepNum ? 'var(--primary)' : 'var(--border)',
                          color: i < stepNum ? 'white' : 'var(--text-muted)',
                      ...
                      {i < steps.length - 1 && (
                        <div style={{
                          ...
                          background: i < stepNum - 1 ? 'var(--primary)' : 'var(--border)',
                          borderRadius: 2
                        }} />
                      )}
                    </div>
              ...
              <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '16px', marginBottom: 24, border: '1px solid var(--border)' }}>
                <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 12 }}>
                  UPDATE ORDER STATUS
                </p>
                  ...
                  {[
                    ...
                    <button key={s.value} onClick={() => updateStatus(s.value)}
                      ...
                      style={{
                        ...
                        fontWeight: 600, cursor: selected.status === s.value ? 'default' : 'pointer',
                        border: `2px solid ${s.color}`,
                        background: selected.status === s.value ? s.color : 'white',
                        color: selected.status === s.value ? 'white' : s.color,
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: '16px', marginTop: 16 }}>
                <span>Total</span>
